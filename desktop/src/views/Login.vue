  ...
  <div class="container">
    ...
    >
      ...
      <a-form-item
          ...
          name="code"
      >
        <a-input v-model:value="loginForm.code" class="form-input" placeholder="验证码"/>
      </a-form-item>
...
export default {
  ...
  methods: {
    saveHostConfig() {
      ...
      if (this.hostConfig.http_port) {
        fsStore.set('http_port', this.hostConfig.http_port)
      }
      if (this.hostConfig.websocket_port) {
      ...
      if (this.ifCode) {
        login(this.loginForm).then(response => {
          if (response.data.code !== 0) {
            ...
            fsStore.set('token', response.data.data.token)
            fsStore.set('username', response.data.data.username)
